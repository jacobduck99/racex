...
def brake_marker_coaching(corners, lap_dist):
    ...
    for brake_zones in corners:
        ...
        if fast_brake_zone is not None and ref_brake_zone is not None:
            if fast_brake_zone.brake_on_pc > 1 ref_brake_zone.brake_on_pct:
                braking_same = {"braking": f"You are braking the same as your fastest lap sector {brake_zones.corner_num}"}
                brake_tips.append(braking_same)
